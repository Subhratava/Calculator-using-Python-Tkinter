from tkinter import E, INSERT, W, Tk,Entry,END,Toplevel,Text,Button,Grid,Scrollbar
...
def button_add(number):
    global resu
    if resu == True and (e.index(INSERT) == e.index(END)):
        clr()
        resu = False
    e.insert(INSERT, number)
...
def evaluate(event=None):
    try:
        ...
        e.insert(END,temp)
        # print(temp)
        #print(vulneribilty_check(iter(temp)))
...
def hst():
    ...
    Scroll = tkscrolled.ScrolledText(history, width=40, height=10, wrap='word',font=('Arial 12'))
    def Clear_hist():
        global hist
        hist = []
        Scroll.delete('1.0', END)
    button_clrh = Button(history, text="Clear History",command=lambda: Clear_hist())
    button_clrh.pack(expand=True, fill='both')
    #w = Text(history, borderwidth=0,font=('Arial 12'))
    for i in range(len(hist)):
        Scroll.insert(END, str(hist[i]) + '\n')
    Scroll.pack()
